...
/* Se incluye librería para transformar el string json recibido por mqtt, transformarlo en json y modificar instrucciones */
...
class MQTT {
  ...
  public:
    ...
    void publish_MQTT ( const char* );              /* Conecta al usuario como un pulicador MQTT */
    ...
    void set_MQTT_callback ( void (*func)(char*, byte*, unsigned int) );         /* Se configura la obtención de la información como suscriptor */
...
void MQTT :: set_MQTT_callback ( void (*func)(char*, byte*, unsigned int) ) {

  client.setCallback (func);                 /* Función librería PubsubClient */
...
void MQTT :: subscribe_MQTT ( void ) {
  ...
  //client.subscribe( RXTOPIC1 );                          /* Función librería PubsubClient */ //No se necesita 2 topics
...
void MQTT :: publish_MQTT (const char* jsonString) {
  ...
  if (client.connected()) {
    client.publish(TXTOPIC, jsonString);
    Serial.println("JSON publicado en el tema MQTT");
  } else {
    Serial.println("No se puede publicar el JSON, no hay conexión MQTT");
  }
}
...
//  client.username_pw_set(USERNAME, USERPASS);
